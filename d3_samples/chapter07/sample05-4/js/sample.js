...
barElements.enter()	// データ数だけ繰り返す
	...
	.attr("width", 20)	// 横幅を指定
	.attr("x", function(d, i){
		return i * 25+offsetX;		// X座標を出現順番×25+offsetXにする
	})
...
barElements.enter()	// text要素を指定
	...
	.attr("x", function(d, i){	// X座標を指定する
		return i * 25 + 10+offsetX;	// 棒グラフの表示間隔に合わせる
	})
...
d3.select("#myGraph")
	...
	.call(
		...
		.ticks(10)	// 目盛り間隔
		.tickValues([10.5, 20, 30.25, 50, 100.75, 150, 200])	// 目盛り間隔
		.tickFormat(d3.format(".2f"))	// 書式
	)
// 横方向の線を表示する
d3.select("#myGraph")
	.append("rect")
