...
d3.html("mydata.html", function(error, docFragment){
	var tr = docFragment.querySelectorAll("table tr");	// table要素からtr要素を抽出
	var dataSet = [ ];	// データを格納する配列を用意する
	for(var i=1; i<tr.length; i++){	// tr要素の行数分-1だけ繰り返す(1行目のヘッダーは無視)
		var d = tr[i].querySelectorAll("td")[0].firstChild.nodeValue;	// データ読み出し
		dataSet.push(d);	// 商品Aのデータだけ抽出する
	}
	// グラフを描画
	d3.select("#myGraph")
